from flask import send_file,Flask,abort, send_from_directory, Response, jsonify, session, redirect, url_for, flash, render_template, request,send_from_directory
from models import *
...
# Utility function to handle video prediction
...
def predict():
    ...
    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
        ...
        return send_file(output_image_path, as_attachment=True, download_name='output_image.jpg')
        ...
        return send_file(output_video_path, as_attachment=True, download_name='output_video.mp4')

    else:
        return jsonify({'error': 'Unsupported file format! Only image and video files are accepted.'}), 400

UPLOAD_FOLDER = 'uploads'
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

@app.route('/analyze_fight', methods=['POST'])
def analyze_fight():
    ...
    # Here you would normally process the video and return the result.
    # For this example, we will just send back the same video file.
    return send_file(video_path, as_attachment=True)

@app.route('/assess_disaster', methods=['POST'])
def assess_disaster():
    ...
    # Here you would normally process the image and return a result.
    # For this example, we'll just return a sample text response.
    return jsonify({'message': 'Disaster assessment complete. No issues detected.'})
